...
@dataclass
class PrefMoEMOELoraConfig(LoraConfig):
    """
    This is the configuration class to store the configuration of a [`~peft.MOE_LORA_PrefMoE`]
    """
    task_embedding_dim: int = field(default=64)
    expert_num: int = field(default=4)

    def __post_init__(self):
        self.peft_type = PeftType.MOE_LORA_PrefMoE


class PrefMoEMOELoraModel(LoraModel):
    """
    ...
    def add_adapter(self, adapter_name, config=None):
        if config is not None:  # get the lora config
            ...
            config = self._prepare_prefmoelora_config(config, model_config)   # load config
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = PrefMoEMOELoraLinear(adapter_name, in_features, out_features, 
                                                    bias=bias, **kwargs)
    ...
    @staticmethod
    def _prepare_prefmoelora_config(peft_config, model_config):
...
class PrefMoEMOELoraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A.update(nn.ModuleDict({adapter_name: PrefMoEMOELinearA(self.in_features, r, self.expert_num)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: PrefMoEMOELinearB(r, self.out_features, self.expert_num)}))
...
class PrefMoEMOELoraLinear(nn.Linear, PrefMoEMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        PrefMoEMOELoraLayer.__init__(self, in_features=in_features, 
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.disable_adapters:   # No adapter
            ...
            task_file = os.environ.get("PREFMOE_TASK_FILE", "data/mmpb_clean/task.txt")
            try:
            ...
            flattened_tensor = router_topk_indices.cpu().flatten()
            unique_values, counts = np.unique(flattened_tensor, return_counts=True)
            if not osp.exists(txt_file_path):
...
class PrefMoEMOELinearA(nn.Module):
    ...
    def __init__(self, in_features, out_features, expert_num) -> None:
        ...
        for _ in range(self.expert_num):
            self.loraA.append(PrefMoEMOEExpert(self.in_features, self.r))
...
class PrefMoEMOELinearB(nn.Module):
    '''MMOE based LoRA block'''
    def __init__(self, in_features, out_features, expert_num) -> None:
        ...
        for _ in range(self.expert_num):
            self.loraB.append(PrefMoEMOEExpert(self.r, self.out_features))
...
class PrefMoEMOEExpert(nn.Module):
...
class PrefMoEMOEGate(nn.Module):
...
class PrefMoEMOERouter(nn.Module):
    """
    ...
    def __init__(self, config: PrefMoEMOELoraConfig):
        super().__init__()
